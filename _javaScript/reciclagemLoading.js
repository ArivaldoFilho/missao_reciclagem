...
function setup() {
    ...
    if(loading){
    //=====================LOAD ELEMENTS======================
    userImage = loadImage('_images/UNDB.png', loadElement);
...
//scoreboard
  ...
  textFont(boogaloo);   
  //logo_undb    
  image(userImage, width - width * 0.22, height * 0.01, width * 0.1, height * 0.08);
  //nível
  text('Missão', width - width * 0.07, height * 0.05);
  text('Reciclagem!', width - width * 0.09, height * 0.09);
  //pontuação
  text('Pontuação: ', width-width*0.16, height*0.15);
  text(score, width-width*0.08, height*0.15);
//fim scoreboard
